      ...
      <div className="container mx-auto px-6 lg:px-12 max-w-2xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <div className="flex items-end justify-between mb-12">
            <div>
              <p className="text-[10px] uppercase tracking-[0.5em] text-white/30 mb-2">Help Center</p>
              <h1 className="text-4xl font-bold uppercase tracking-widest text-white">Support</h1>
            </div>
            <button
              onClick={() => setShowCreate(true)}
              className="flex items-center gap-2 px-4 py-2.5 border border-white/20 text-xs uppercase tracking-[0.35em] text-white/60 hover:text-white hover:border-white/50 transition-colors"
            >
          ...
          {ticketsLoading ? (
            ...
            <div className="text-center py-20">
              <p className="text-white/20 text-xs uppercase tracking-[0.5em] mb-4">No tickets yet</p>
              <p className="text-white/30 text-sm mb-8">Have a question or issue? We're here to help.</p>
              <button onClick={() => setShowCreate(true)}
                className="px-6 py-3 border border-white/20 text-xs uppercase tracking-[0.35em] text-white/50 hover:text-white hover:border-white/50 transition-colors">
                Open a Ticket
              </button>
            </div>
          ) : (
            <div className="space-y-3">
              {tickets.map((ticket) => (
                <button
                  ...
                  className="w-full border border-white/8 p-5 text-left hover:border-white/20 transition-colors flex items-start justify-between gap-4"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white font-medium truncate">{ticket.subject}</p>
                    <p className="text-xs text-white/30 mt-1">{new Date(ticket.created_at).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p>
                  </div>
        ...
        {showCreate && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-x-4 top-1/2 -translate-y-1/2 md:inset-auto md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-lg bg-[#0a0a0a] border border-white/10 p-8 z-50"
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-sm font-semibold text-white uppercase tracking-widest">New Ticket</h3>
                <button onClick={() => setShowCreate(false)} className="text-white/30 hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <form onSubmit={createTicket} className="space-y-4">
                <div>
                  <label className="block text-[10px] uppercase tracking-[0.4em] text-white/40 mb-2">Subject</label>
                ...
                <div>
                  <label className="block text-[10px] uppercase tracking-[0.4em] text-white/40 mb-2">Message</label>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <div className="bg-white/5 rounded p-4">
                  <p className="text-[10px] uppercase tracking-widest text-white/30 mb-2">Your message</p>
                  <p className="text-sm text-white/80 leading-relaxed">{activeTicket.message}</p>
                </div>
                {ticketMessages.map((msg) => (
                  <div key={msg.id} className={`p-4 rounded ${msg.sender_role === "worker" ? "bg-white/8 ml-4 border-l-2 border-blue-400/20" : "bg-white/5 mr-4"}`}>
                ...
                {tickets.find((t) => t.id === activeTicket.id)?.status === "closed" && (
                  <p className="text-center text-xs text-white/25 uppercase tracking-widest py-4">This ticket is closed</p>
                )}
              </div>
