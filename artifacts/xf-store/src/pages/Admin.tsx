      ...
      <div className="container mx-auto px-6 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <div className="flex flex-col md:flex-row md:items-end gap-6 mb-12">
            <div>
              <p className="text-[10px] uppercase tracking-[0.5em] text-white/30 mb-2">Admin Panel</p>
              <h1 className="text-4xl font-bold uppercase tracking-widest text-white">Orders</h1>
            </div>
          ...
          <div className="mb-6 flex items-center gap-3">
            <span className="text-xs text-white/30 uppercase tracking-widest">{filtered.length} orders</span>
          </div>
                ...
                <div key={order.id} className="border border-white/8 p-6">
                  <div className="flex flex-col lg:flex-row lg:items-start gap-6">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-white/40 tracking-wide mb-3">{order.shipping_address}</p>
                      {order.order_items && (
                    ...
                    <div className="flex-shrink-0 flex flex-col gap-2">
                      <span className={`text-[10px] uppercase tracking-[0.35em] px-3 py-1.5 border ${STATUS_COLORS[order.status as Status] || "text-white/50 border-white/20"} text-center`}>
                        {order.status}
                      </span>
                      <select
                        value={order.status}
